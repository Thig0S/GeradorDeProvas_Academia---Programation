...
public sealed class ServicoProva(
    IRepositorioProva repositorioProva,
...
) : ServicoBase<Prova>
{
    ...
    {
        List<Guid> idsOrdenados = ids.ToList();
    ...
    private Result<Prova> PrepararProva(
    ...
    )
    {
        if (ExisteProvaComMesmoTitulo(dto.Titulo))
        {
            return Result.Fail<Prova>(
                new Error("Já existe uma prova com este título.")
                    .WithMetadata("Campo", nameof(dto.Titulo))
            );
        }
        ...
        Result<Materia?> resultadoMateria = SelecionarMateria(
            dto.DisciplinaId,
            dto.MateriaId,
    ...
    private bool ExisteProvaComMesmoTitulo(string titulo, Guid? idIgnorado = null)
    {
        ...
        return repositorioProva
            .SelecionarTodos()
            .Any(p => p.Id != idIgnorado && NormalizarTitulo(p.Titulo) == tituloNormalizado);
    }
    ...
    private static DetalhesProvaDto MapearDetalhes(Prova prova)
    {
        return new DetalhesProvaDto(
            ...
            prova.Questoes.Select(q => new QuestaoProvaDto(
                q.Id,
                q.Enunciado,
                q.Alternativas.Select(a => new AlternativaProvaDto(a.Id, a.Texto, a.Correta)).ToList()
            )).ToList()
        );
    }
    ...
    private Result<Materia?> SelecionarMateria(Guid disciplinaId, Guid? materiaId, bool recuperacao)
    {
        ...
        {
            return Result.Fail<Materia?>(
                new Error("A matéria selecionada não pertence à disciplina informada.")
                    .WithMetadata("Campo", nameof(CadastrarProvaDto.MateriaId))
            );
        }
    ...
    {
        List<Guid> idsMaterias = materiasElegiveis.Select(m => m.Id).ToList();
    ...
    )
    {
        ...
        {
            return Result.Fail<List<Questao>>(
                new Error("A quantidade de questões confirmada é inválida.")
                    .WithMetadata("Campo", nameof(CadastrarProvaDto.QuantidadeQuestoes))
            );
        }

        HashSet<Guid> idsMaterias = materiasElegiveis.Select(m => m.Id).ToHashSet();
        List<Questao> questoes = repositorioQuestao
            .SelecionarTodos()
        ...
        {
            return Result.Fail<List<Questao>>(
                new Error("Uma ou mais questões confirmadas não pertencem à configuração da prova.")
                    .WithMetadata("Campo", nameof(CadastrarProvaDto.QuantidadeQuestoes))
            );
        }
    ...
    private static QuestaoProvaDto MapearQuestao(Questao questao)
    {
        return new QuestaoProvaDto(
            questao.Id,
            questao.Enunciado,
            questao.Alternativas.Select(a => new AlternativaProvaDto(a.Id, a.Texto, a.Correta)).ToList()
        );
    }
}